...
import sendEstimateAsPdf from '@salesforce/apex/QB_CalloutUtility.sendEstimateAsPdfFromQuickbook';
...
export default class Qb_SendEmailAndPDF extends LightningElement {
    @api recordId;
    @api objectApiName;
        ...
        .then(result => {
           ...
           this.qbId = arr[2];
            if(this.qbEntity === 'Estimate' ){
                this.qbButtonLabel = 'Send Estimate as an Email and PDF';
            }
           }
        });
    ...
    handleClickDownload(){
        ...
        this.showErrorToast('Company Id Or Qb Id is not found');
            ...
            if(isValidEmail) {
                ...
                sendEstimateAsPdf({'companyId': this.companyId,'qbEntity': this.qbEntity,'qbId':this.qbId,'specificEmail':this.specificEmail})
                .then((result) => {
                    this.isLoaded = true;
                    if(result){
    ...
    //     }
    //     catch(ex){
    //         //console.log.log('in final err',ex.message);
    //         this.isLoaded = true;
